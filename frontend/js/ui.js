...
function shell(body,seg){
 ...
 <h6>Reps${isRep?'':`<button data-go="/settings/reps" aria-label="Manage reps">${ic('plus',15)}</button>`}</h6>${reps}
...
function pgOverview(){
 ...
 <div class="acts"><button class="pill live" data-go="/activity" style="border:0;cursor:pointer;height:32px">${ic('spark',13)}Agents finished ${done} jobs today</button>${S.crmUrl?`<a class="btn" href="${S.crmUrl}" target="_blank" rel="noopener">${ic('ext',16)}Open CRM</a>`:''}${me.role!=='Rep'?`<button class="btn pri" data-go="/campaigns/new">${ic('plus',16)}New campaign</button>`:''}</div></div>
...
function pgCampaigns(){
 ...
 <div class="pad"><div class="filters" style="margin-bottom:16px"><div class="seg">${['all','live','paused','draft','completed','archived'].map(k=>`<button class="${f===k?'on':''}" data-a="campFil" data-k="${k}">${cap(k)}</button>`).join('')}</div><input class="inp" style="max-width:260px" placeholder="Search campaigns" data-i="campq" value="${esc(UI.fil.campq||'')}"></div>
...
function campMenu(c){return`<div class="menu"><button data-a="dup" data-id="${c.id}">${ic('copy',15)}Duplicate as variant</button>${c.status==='live'||c.status==='paused'?`<button data-a="complete" data-id="${c.id}">${ic('check',15)}Complete</button>`:''}${c.status!=='archived'?`<button data-a="archive" data-id="${c.id}">${ic('trash',15)}Archive</button>`:S.user.role==='Admin'?`<button data-a="delCamp" data-id="${c.id}" style="color:var(--kill)">${ic('trash',15)}Delete permanently</button>`:''}</div>`}
...
function pgProspect(pid){
 ...
 <div class="col gap16"><div class="card"><div class="cardhead"><div class="h3">Profile</div></div><dl class="kv" style="grid-template-columns:78px 1fr"><dt>Email</dt><dd class="small" style="word-break:break-all">${esc(p.email)}</dd><dt>LinkedIn</dt><dd class="small">${esc(p.linkedin)}</dd><dt>Phone</dt><dd class="small">${esc(p.phone)}</dd><dt>Company</dt><dd class="small">${esc(p.ind)||'Existing customer'}, ${p.staff.toLocaleString('en-US')} staff${p.stage&&p.stage!=='n/a'?', '+esc(p.stage):''}</dd><dt>Rep</dt><dd class="small">${S.user.role==='Rep'?esc(rep.name):`<select class="sel sm" style="width:100%" data-i="enrRep" data-id="${e.id}">${S.users.filter(u=>u.role==='Rep'&&(u.active||u.id===rep.id)).map(u=>`<option value="${u.id}" ${u.id===rep.id?'selected':''}>${esc(u.name)}${u.active?'':' (offboarded)'}</option>`).join('')}</select>`}</dd></dl>${p.bio?`<div class="banner bad" style="margin-top:12px;font-weight:450;align-items:flex-start">${ic('shield',16)}<span>Bio contains instruction-like text. The Guardian strips it before any model reads it: <i>"${esc(p.bio)}"</i></span></div>`:''}</div>
...
function activityView(cid){
 ...
 const c=pickCamp(cid||F.swc);if(!c)return`<div class="pad">${empty('No campaigns yet','Create a campaign to see agent activity.')}</div>`;const swc=c.id;
...
function promptsView(cid){
 const rep=S.user.role==='Rep';
 const c=cid?C(cid):pickCamp(UI.fil.prc);if(!c)return`<div class="pad">${empty('No campaigns yet','Create a campaign to see its prompts.')}</div>`;const P_=UI.pr=UI.pr||{};if(P_.cid!==c.id){P_.cid=c.id;P_.role='Writer';P_.v=null;P_.cmp=false}
...
function knowledgeView(cid){
 const c=cid?C(cid):pickCamp(UI.fil.kbc);if(!c)return`<div class="pad">${empty('No campaigns yet','Create a campaign to see its knowledge base.')}</div>`;const rep=S.user.role==='Rep';
...
function pgSettings(tab){
 ...
 if(tab==='integrations'){
  ...
  body=`<div class="pad"><div class="grid g2">${Object.entries(S.integ).map(([k,v])=>`<div class="card"><div class="cardhead"><div class="row"><b style="font-size:16px">${v.n}</b>${v.status!=='ok'?'<span class="pill bad">'+ic('alert',12)+'Error</span>':v.canLive?'<span class="pill live"><i class="dot live"></i>Connected</span>':'<span class="pill draft">Not connected</span>'}${v.paused?'<span class="pill paused">Paused</span>':''}</div>${chMap[k]?`<div class="row"><span class="small muted">Enabled</span>${sw(!v.paused,'integSw','data-k="'+k+'"')}</div>`:''}</div><div class="muted small">${v.d}</div>${v.err?`<div class="banner bad" style="margin-top:12px;font-weight:450;align-items:flex-start">${ic('alert',16)}<span>${esc(v.err)}</span></div>`:''}<div class="row wrap" style="margin-top:14px"><span class="small faint">Last check ${rel(v.last)}</span><span class="right"></span>${v.canLive?`<div class="seg"><button class="${v.mode==='live'?'on':''}" data-a="integMode" data-k="${k}" data-m="live">Live</button><button class="${v.mode==='sandbox'?'on':''}" data-a="integMode" data-k="${k}" data-m="sandbox">Sandbox</button></div>`:'<span class="tag sbx">SANDBOX ONLY</span>'}<button class="btn sm" data-a="integTest" data-k="${k}">Test connection</button></div></div>`).join('')}</div></div>`;
   ...
   const canManage=['Admin','Manager'].includes(S.user.role);
   body=`<div class="pad"><div class="row" style="margin-bottom:14px"><span class="muted">Reps receive escalations and own the meetings the agents book. Admins and managers add reps, managers and other admins.</span><span class="right"></span>${canManage?`<button class="btn pri" data-a="repAdd">${ic('plus',15)}Add user</button>`:''}</div><div class="tblwrap"><table class="tbl"><thead><tr><th>Person</th><th>Role</th><th>Hours</th><th>Daily limit</th><th>Channels</th><th>Campaigns</th><th>Status</th><th></th></tr></thead><tbody>${S.users.map(u=>`<tr><td><div class="row">${av(u.name,'',u.active)}<div><b style="font-weight:550">${esc(u.name)}</b><div class="small muted">${esc(u.title)}${u.label?', '+u.label:''}</div></div></div></td><td>${u.role}</td><td class="small">${u.hours} ${u.tz}</td><td>${u.role==='Rep'?`<input class="inp sm" type="number" min="0" style="width:80px" data-i="repLim" data-id="${u.id}" value="${u.limit}">`:'-'}</td><td class="small">${u.channels.map(c=>CH[c].n).join(', ')}</td><td>${S.camps.filter(c=>c.reps.includes(u.id)).map(c=>campChip(c.id)).join('')||'<span class="faint">None</span>'}</td><td>${u.active?'<span class="pill live">Active</span>':'<span class="pill done">Offboarded</span>'}</td><td class="tar"><span class="row gap8" style="justify-content:flex-end">${canManage?`<button class="btn xs" data-a="repEdit" data-id="${u.id}">${ic('edit',13)}Edit</button>`:''}${canManage&&u.role==='Rep'&&u.active?`<button class="btn xs" data-a="repOff" data-id="${u.id}">Offboard</button>`:''}${canManage&&u.role==='Rep'?`<button class="btn xs" data-a="repDel" data-id="${u.id}">${ic('trash',13)}Delete</button>`:''}</span></td></tr>`).join('')}</tbody></table></div></div>`;
  ...
  <div class="card"><div class="h3">Reset demo data</div><p class="muted small" style="margin:6px 0 14px">Rebuilds the seeded workspace. Every change you made goes away.</p><button class="btn dan" data-a="reset">Reset data</button></div></div>
  <div class="banner warn" style="font-weight:450">${ic('alert',16)}This one is real, not a demo tool. It automates a personal LinkedIn account to find people, which is against LinkedIn's terms — see MANUAL_ACTIONS.md MA-14 before running it.</div>
  <div class="card"><div class="h3">Find real employees on LinkedIn</div><p class="muted small" style="margin:6px 0 14px">Downloads a script that searches a real company's LinkedIn people results using your own logged-in Chrome session, then adds real prospects (real name, title, LinkedIn URL) to a campaign. Runs only on your own machine — this server has no browser and never runs it.</p><a class="btn" href="/tools/linkedin-finder.js" download>Download finder script</a></div></div>`;
 }
...
function onField(e){
 ...
 if(i==='enrRep'&&!isInput){run(()=>api.post('/enrollments/'+el.dataset.id+'/reassign',{replacement_rep_id:el.value}),{ok:()=>`Reassigned to ${U(el.value).name}.`});return}
}
